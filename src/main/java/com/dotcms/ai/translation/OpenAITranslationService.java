...
public class OpenAITranslationService extends AbstractTranslationService {
   ...
   @Override
   public Contentlet translateContent(Contentlet contentlet, Language targetLanguage,
           List<com.dotmarketing.portlets.structure.model.Field> oldFields, User user)
           throws TranslationException {
      ...
      Optional<String> translationKeyPrefix = Optional.ofNullable(
              (String) contentlet.getMap().get(OpenAITranslationActionlet.TRANSLATION_KEY_PREFIX));

      JSONObject translationKeysJSON = new JSONObject(
              getTranslationKeys(translationKeyPrefix, contentlet.getLanguageId(), targetLanguage.getId()));
   ...
   Optional<String> getResponseFormat(String hostId) {
      ...
      return Optional.of("json_object");
   }
